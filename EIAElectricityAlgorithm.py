...
class EIAElectricityAlgorithm(QCAlgorithm):
    ...
    algorithm trades on load surprise: hours where actual demand runs above what PJM forecast a day
    ahead mean the grid is tighter than the market expected. Grid data is a signal feed (not
    ...
    def on_data(self, slice):
        ...
        self.debug(f"{self.time:%Y-%m-%d %H:%M} PJM - demand: {grid.demand}, forecast: {grid.demand_forecast}, "
        ...
        if load_surprise > 0.02:
            self.set_holdings(self._xlu, 1)
        elif load_surprise < -0.02:
            self.liquidate(self._xlu)
