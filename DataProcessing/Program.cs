    ...
    public class Program
    {
        ...
        public static void Main()
        {
            ...
            var destinationDirectory = Path.Combine(
                ...
                "alternative",
                EIAElectricityDownloader.VendorName);

            // The published history that an incremental run merges its new data into.
            var processedDataDirectory = Path.Combine(
                Config.Get("processed-data-directory", Globals.DataFolder),
                "alternative",
                EIAElectricityDownloader.VendorName);

            EIAElectricityDownloader instance = null;
            try
            {
                // Pass in the values we got from the configuration into the downloader/converter.
                instance = new EIAElectricityDownloader(destinationDirectory, processedDataDirectory);
            }
            catch (Exception err)
            {
                Log.Error(err, $"QuantConnect.DataProcessing.Program.Main(): The downloader/converter for {EIAElectricityDownloader.VendorName} {EIAElectricityDownloader.VendorDataName} data failed to be constructed");
                Environment.Exit(1);
            }
            ...
            try
            {
                ...
                if (!success)
                {
                    Log.Error($"QuantConnect.DataProcessing.Program.Main(): Failed to download/process {EIAElectricityDownloader.VendorName} {EIAElectricityDownloader.VendorDataName} data");
                    Environment.Exit(1);
                }
            }
            catch (Exception err)
            {
                Log.Error(err, $"QuantConnect.DataProcessing.Program.Main(): The downloader/converter for {EIAElectricityDownloader.VendorName} {EIAElectricityDownloader.VendorDataName} data exited unexpectedly");
                Environment.Exit(1);
            }
            finally
            {
